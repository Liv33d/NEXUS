...
const [html, javascript, css, earthTexture] = await Promise.all([
  ...
  readFile(resolve('public/earth-texture.svg'), 'utf8'),
])

const earthDataUrl = `data:image/svg+xml,${encodeURIComponent(earthTexture)}`
const inlinedJavascript = javascript.replaceAll('./earth-texture.svg', earthDataUrl).replaceAll('</script>', '<\\/script>')
const preview = html
