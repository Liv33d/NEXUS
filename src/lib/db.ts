...
export async function pruneDatabase(now = Date.now()): Promise<void> {
  ...
  await db.transaction('rw', db.signals, db.discoveries, db.cache, db.memory, async () => {
...
export async function eraseDatabase(): Promise<void> {
  await db.transaction('rw', [db.signals, db.discoveries, db.providerStatus, db.settings, db.cache, db.watches, db.memory], async () => {
    await Promise.all([db.signals.clear(), db.discoveries.clear(), db.providerStatus.clear(), db.settings.clear(), db.cache.clear(), db.watches.clear(), db.memory.clear()])
  })
}
