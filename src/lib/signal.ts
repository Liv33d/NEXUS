...
export const signalSchema = z.object({
  ...
  source: z.object({
    ...
    sourceFamily: z.string().min(1).max(120).optional(),
    sourceRole: z.enum(['primary-observation', 'official-product', 'aggregator', 'administrative', 'derived']).optional(),
    upstreamKey: z.string().min(1).max(240).optional(),
    revisionKey: z.string().min(1).max(240).optional(),
    upstreamRefs: z.array(z.object({ sourceFamily: z.string().min(1).max(120), upstreamKey: z.string().max(240).optional(), url: safeUrl.optional() })).max(30).optional(),
  }),
  ...
  temporal: z.object({
    ...
    precision: z.enum(['second', 'minute', 'hour', 'day', 'unknown']),
    basis: z.enum(['sensor-observation', 'event-occurrence', 'publisher-issue', 'product-validity', 'current-state-confirmation', 'retrieval-fallback', 'legacy-unknown']),
  }).optional(),
  location: z.object({
