...
import { fetchGbifTaxonPresentation } from './gbifPresentation'
import { signalTemporal, temporalDistanceMs } from './temporal'
...
function sourceStatus(signal: Signal): NexusIntelligenceObject['status'] {
  ...
  if (signal.source.freshness === 'delayed') return 'recent'
  const age = Date.now() - signalTemporal(signal).effectiveAt
  return age < 15 * 60_000 ? 'near-real-time' : 'recent'
}

type ThermalClassification = 'unclassified' | 'possible-fire' | 'possible-volcanic'
...
export function classifyThermalSignal(signal: Signal, evidence: Signal[]): { classification: ThermalClassification; related: Signal[] } {
  ...
  const nearby = evidence.filter((candidate) => candidate.id !== signal.id && candidate.location && temporalDistanceMs(candidate, signal) <= 24 * 3_600_000 && distanceKm(signal.location!, { lat: candidate.location!.latitude, lng: candidate.location!.longitude }) <= 10)
...
export function lifeTaxonToIntelligence(taxon: LifeGlobeTaxon, retrievedAt: number, methodology: string, freshness: 'live' | 'cached' = 'live'): NexusIntelligenceObject {
  ...
  return {
    ...
    subtitle: `${taxon.taxonomicClass ?? taxon.kingdom ?? 'LIFE'} · PUBLISHED OBSERVATIONS`, status: freshness === 'cached' ? 'cached' : 'recent', evidence: 'observed', timestamp: retrievedAt,
    location: { latitude: taxon.latitude, longitude: taxon.longitude },
    media: taxon.media ? [{ id: `${taxon.id}-photo`, kind: 'photo', url: taxon.media.url, title: commonName, alt: `Representative photograph of ${commonName}`, creator: taxon.media.creator, license: taxon.media.license, attribution: `${taxon.media.creator} · ${taxon.media.license} · ${taxon.media.source}`, sourceUrl: taxon.media.sourceUrl, freshness: 'historical' }] : [],
    summary: `${taxon.observations.toLocaleString()} permissively licensed recent observation${taxon.observations === 1 ? '' : 's'} in the current bounded regional sample.`,
    ...
    relationships: [], provenance: [{ label: 'OPEN_DATA', description: 'GBIF occurrence and species services; only CC0 occurrence records are used in spatial aggregates.' }],
    methodology, sourceUrl: taxon.sourceUrl, watchLabel: 'Watch species',
  }
}

export function ecologicalClusterToIntelligence(cell: LifeGlobeCell, retrievedAt: number, methodology: string, freshness: 'live' | 'cached' = 'live'): NexusIntelligenceObject {
  return {
    id: `life-cluster-${cell.id}`, kind: 'life-cluster', domain: 'life', title: `${cell.observations.toLocaleString()} life observation records`,
    subtitle: 'COARSE BIODIVERSITY AREA', status: freshness === 'cached' ? 'cached' : 'recent', evidence: 'observed', timestamp: retrievedAt,
    ...
    provenance: [{ label: 'DERIVED_METRIC', description: 'Coarse H3 aggregation of permissively licensed GBIF occurrence records.' }], methodology, watchLabel: 'Watch this area',
  }
}

export function signalClusterToIntelligence(signals: Signal[], location: { latitude: number; longitude: number }, totalCount = signals.length): NexusIntelligenceObject {
  ...
  return {
    ...
    title: `${totalCount} signals here`, subtitle: 'LOCAL ACTIVITY CLUSTER', status: 'recent', timestamp: newest,
    location, media: [],
    summary: `NEXUS grouped ${totalCount} nearby signals to keep the map readable.${totalCount > ranked.length ? ` The ${ranked.length} sampled items below represent this cluster.` : ' The most significant items are listed below.'}`,
    whyItMatters: 'A cluster is a visual summary, not a single event. Select an item to inspect its evidence, context, and source.',
    facts: [{ label: 'Signals', value: totalCount.toLocaleString() }, { label: 'Sample shown', value: ranked.length.toLocaleString() }, { label: 'Data domains', value: domains.size.toLocaleString() }],
...
export function discoveryToIntelligence(discovery: Discovery, signals: Signal[]): NexusIntelligenceObject {
  ...
  return {
    ...
    facts: [
      ...
      { label: 'Independent sources', value: new Set(ranked.flatMap((signal) => signal.source.upstreamRefs?.length ? signal.source.upstreamRefs.map((reference) => reference.sourceFamily) : signal.source.sourceRole === 'aggregator' ? [] : [signal.source.sourceFamily ?? signal.source.provider])).size.toLocaleString() },
    ],
...
export function observerTaxonToIntelligence(taxon: LifeTaxonSummary, context: LifeContext, location: { latitude: number; longitude: number }): NexusIntelligenceObject {
  ...
  return {
    ...
    relationships: [], provenance: [{ label: 'OPEN_DATA', description: 'GBIF occurrence and species services; NEXUS accepts only CC0 occurrence records for this spatial aggregate.' }],
...
export async function enrichSelectedIntelligence(object: NexusIntelligenceObject, signal?: AbortSignal): Promise<NexusIntelligenceObject> {
  if (object.kind === 'species' && !object.media.length) {
    const taxonKey = Number(object.sourceUrl?.match(/\/species\/(\d+)/)?.[1])
    if (Number.isFinite(taxonKey)) {
      const presentation = await fetchGbifTaxonPresentation(taxonKey, object.sourceUrl!, signal)
      if (presentation.media) {
        const media = presentation.media
        return { ...object, title: presentation.commonName ?? object.title, media: [{ id: `${object.id}-photo`, kind: 'photo', role: 'representative', url: media.url, title: presentation.commonName ?? object.title, alt: `Representative photograph of ${presentation.commonName ?? object.title}`, creator: media.creator, license: media.license, attribution: `${media.creator} · ${media.license} · ${media.source}`, sourceUrl: media.sourceUrl, freshness: 'historical' }] }
      }
    }
    return object
  }
