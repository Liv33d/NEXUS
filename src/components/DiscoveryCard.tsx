...
export function DiscoveryCard({ discovery, index, onOpen, onSave }: { discovery: Discovery; index: number; onOpen(): void; onSave(): void }) {
  return (
    <article className={`discovery-card level-${discovery.level}`}>
      ...
      <div className="discovery-meta"><span><Network size={14}/>{discovery.signalIds.length} signals</span><span><Clock3 size={14}/>{new Date(discovery.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span></div>
