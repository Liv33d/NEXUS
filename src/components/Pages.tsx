import { Activity, BellRing, Bird, BookmarkCheck, CloudSun, Compass, Download, ExternalLink, Gauge, LocateFixed, MapPin, Navigation, NotebookPen, Orbit, Radio, Search, ShieldCheck, Sparkles, Star, Sunrise, Sunset, Thermometer, Trash2, Waves, Wind, X } from 'lucide-react'
...
export function CasesPage({ discoveries, watches, signals, onOpen, onObserve, onUnwatch }: { discoveries: Discovery[]; watches: WatchRule[]; signals: Signal[]; onOpen(id: string): void; onObserve(place: ObserverPlace): void; onUnwatch(latitude: number, longitude: number): Promise<void> }) {
  ...
  return <main className="page"><div className="page-heading"><div><span className="eyebrow">LOCAL & PRIVATE</span><h1>Cases</h1></div></div>{saved.length ? <><h2 className="section-title">Saved investigations</h2><div className="card-stack">{saved.map((discovery, index) => <DiscoveryCard key={discovery.id} discovery={discovery} index={index} onOpen={() => onOpen(discovery.id)} onSave={() => undefined}/>)}</div></> : <EmptyState icon={<ShieldCheck/>} title="No saved investigations">Save a discovery to preserve its evidence locally. Nothing is uploaded.</EmptyState>}<h2 className="section-title">Watched places <small>IN‑APP</small></h2>{watches.length ? <div className="watch-list">{watches.map((watch) => { const match = evaluateWatch(watch, signals); return <div key={watch.id}><BellRing/><button onClick={() => onObserve({ id: watch.id, name: watch.target.name, subtitle: `${watch.conditions.radiusKm} km · severity ${watch.conditions.minimumSeverity}+`, latitude: watch.target.latitude, longitude: watch.target.longitude })}><strong>{watch.target.name}</strong><small>{match.signalIds.length ? `${match.signalIds.length} elevated nearby now` : 'No elevated nearby activity'} · {watch.conditions.radiusKm} km</small></button><button aria-label={`Stop watching ${watch.target.name}`} onClick={() => void onUnwatch(watch.target.latitude, watch.target.longitude)}><X/></button></div> })}</div> : <p className="quiet-copy">Watch a place in Observer to surface elevated nearby Signals whenever NEXUS is open.</p>}</main>
}
