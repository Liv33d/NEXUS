...
export function ObserverPage({ signals }: { signals: Signal[] }) {
  ...
  const [savedPlaces, setSavedPlaces] = useState<ObserverPlace[]>(() => {
    try {
      ...
      return value.filter((item): item is ObserverPlace => Boolean(item && typeof item === 'object' && 'id' in item && 'name' in item && 'latitude' in item && 'longitude' in item && typeof item.id === 'string' && typeof item.name === 'string' && typeof item.latitude === 'number' && typeof item.longitude === 'number' && Math.abs(item.latitude) <= 90 && Math.abs(item.longitude) <= 180)).slice(0, 6)
    } catch { return [] }
  })
