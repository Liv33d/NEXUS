...
export async function runProvider(provider: SignalProvider, context: SignalQueryContext): Promise<Signal[]> {
  // Refreshes deliberately own their transport. Sharing an in-flight promise also
  // shares its AbortSignal, which lets a newer refresh inherit an older aborted
  // request. Provider cadence belongs in the explicit device cache, not here.
